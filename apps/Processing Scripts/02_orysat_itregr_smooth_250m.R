...
CHUNKSIZE = 100000 # In-terms of pixels
...
stk.mndwi <- raster::stack(inv.idxfiles$filename[inv.idxfiles$band=="NDFI"])
...
rm(filelist.index, mdata.lc, stk.mndwi, mat.mndwi, na.count)
gc(reset = TRUE)
...
save(pixels.toprocess, rst.lc, date.acqdates, file=paste0("itregr_", TILE, "_250m_", YEAR, ".rdata"))
rm(water, nwater.maxlen)
gc(reset = TRUE)
