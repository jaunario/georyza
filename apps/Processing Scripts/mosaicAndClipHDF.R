...
setwd(WORKSPACE)
OUTPUT_DIR = paste("./mosaic", COUNTRY, 'indices', sep="/")
...
for (y in YEARS){
  ...
  for(acq in required.acqdates){
    ...
    rst.mosaic <- crop(rst.mosaic, shp.adm)
    message(acq,": Writing to disk.")
    rst.mosaic <- writeRaster(rst.mosaic, filename = paste0(OUTPUT_DIR, "/", paste("SMOOTH", COUNTRY, paste0("A",acq), INDEX_NAME, "500m_WGS", sep="_"), ".tif"), format="GTiff", overwrite=TRUE)
