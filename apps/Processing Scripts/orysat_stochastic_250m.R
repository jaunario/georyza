...
stk.mndwi <- raster::stack(inv.idxfiles$filename[inv.idxfiles$band=="NDH2OI"])
mat.mndwi <- raster::values(stk.mndwi)

#stk.ndfi <- raster::stack(inv.idxfiles$filename[inv.idxfiles$band=="NDFI"])
...
rst.lc <- resample(rst.lc, stk.mndwi, method="ngb")
...
water <- mat.mndwi >= 0 # Optimum threshold for MNDWI to detact surface water according to Boschetti 2014
h20.count <- rowSums(water)
nwater.maxlen <- apply(!water, 1, maxlen.condition)

rst.lc[pixels.toprocess[nwater.maxlen<12]] <- -5 # Permanent water
pixels.toprocess <- pixels.toprocess[nwater.maxlen>=12]
#mat.mndwi <- mat.mndwi[nwater.maxlen>=12,]
rm(stk.mndwi, mat.mndwi, na.count, nwater.maxlen)
gc(reset = TRUE)
...
chunk.st <- seq(1, length(pixels.toprocess), by=CHUNKSIZE)
chunk.en <- pmin(chunk.st-1+CHUNKSIZE, length(pixels.toprocess))

for (i in 1:length(chunk.st)){
  chunktime.st <- Sys.time()
  message("ORYSAT-", METHOD, " CHUNK-", i, ": Extracting EVI.")
  mat.evi <- stk.evi[pixels.toprocess[chunk.st[i]:chunk.en[i]]]
  mat.evi <- t(mat.evi)
  mat.evi <- as.data.frame(mat.evi)
  
  message("ORYSAT-", METHOD, " CHUNK-", i, ": Extracting actual ACQDATES.")
  mat.jday <- stk.jday[pixels.toprocess[chunk.st[i]:chunk.en[i]]]
  mat.jday <- t(mat.jday)
  mat.jday <- as.data.frame(mat.jday)
  
  message("ORYSAT-", METHOD, " CHUNK-", i, ": Smoothing the time-series.")
  ...
  if(i==1) rice <- do.call(rbind,thischunk) else rice <- rbind(rice, do.call(rbind,thischunk))
  
  message("ORYSAT-", METHOD, " CHUNK-", i, ": Cleaning-up.")
  rm(mat.evi, fill.evi, smooth.evi, mat.jday) #, fill.evi
  gc(reset=TRUE)
...
if(length(pixels.toprocess)>1000000){
  ...
  chunks.st <- seq(1,length(pixels.toprocess), by=chunk)
  chunks.en <- c(chunks.st[-1], length(pixels.toprocess))
