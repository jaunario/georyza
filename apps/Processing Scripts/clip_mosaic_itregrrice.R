...
WORKSPACE  = "E:/WORKSPACE/Orysat/250m/IterRegr_0.1_MD13Q1"
OUTPUT_DIR = "AUTO"
YEARS = 2010
COUNTRY  = "BGD" 
METHOD = "IterRegr"
...
setwd(WORKSPACE)
if(OUTPUT_DIR=="AUTO") {
  OUTPUT_DIR <- paste0("./",paste("MOSAIC", COUNTRY, "250m", sep="_"))
...
for (y in YEARS){
  ...
  for (q in 1:4){
    ...
    for (v in unique(dat.files$Variable)){
      ...
      if (v %in% c("EOS","SOS")){
        vals.rice <- as.Date(vals.rice, "1970-1-1")
        mo.rice <- monthFromDate(vals.rice)
        doy.rice <- as.integer(julian(vals.rice,as.Date(paste0(y,"-1-1"))-1))
  ...
  for (i in 1:length(dat.seasons)){
    ...
    rst.sos <- writeRaster(rst.sos, filename=paste0(OUTPUT_DIR,"/", paste("RiceMap",METHOD,COUNTRY, "SOS", y, names(dat.seasons)[i],sep="_"),".tif"), datatype="INT2S")
    rst.eos <- raster(rst.adm)
    rst.eos[dat.s$cell] <- dat.s$EOS_doy
    rst.eos <- writeRaster(rst.eos, filename=paste0(OUTPUT_DIR,"/", paste("RiceMap",METHOD,COUNTRY, "EOS", y, names(dat.seasons)[i],sep="_"),".tif"), datatype="INT2S")
