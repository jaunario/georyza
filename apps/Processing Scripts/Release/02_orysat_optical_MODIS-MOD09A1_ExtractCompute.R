...
for(i in 1:nrow(inv.hdffiles)){
  ...
  for(j in 1:ncol(mdata.INDICES@imgvals)){
    ...
    if(!colnames(mdata.INDICES@imgvals)[j] %in% c("actualdate", "state_flags")){
      if(DO_SPATFILL){
        message("ORYSAT | Extract-Compute,", inv.hdffiles$acqdate[i],": Trying to fill ", INDICES[j])
        thislayer <- focal(thislayer, matrix(rep(1,9),ncol=3), fun=mean, na.rm=TRUE, NAonly=TRUE)
        thislayer[is.nan(thislayer[])] <- NA
        thislayer[water.mask] <- NA
      }
