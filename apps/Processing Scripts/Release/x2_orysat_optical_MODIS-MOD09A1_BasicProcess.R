...
# DIRECTORY SETTINGS
MODIS_HOME = "F:/MODIS"
DEM_DIR     = paste0(MODIS_HOME, "/dem/500m")
...
# METHOD SETTINGS
YEAR       = 2017
METHOD     = "VWI_Dynamics" # Use this to specify which indices are needed instead of making the user to list down the needed indices
...
required.acqdates <- paste0("A",YEAR,sprintf("%03g", ACQDOYS))
  ...
  # paste("A",c(paste(YEAR-1,sprintf("%03g", ACQDOYS)[(length(ACQDOYS)-7):length(ACQDOYS)], sep=""),  #PREVIOUS YEAR
...
rst.dem <- raster(dir(DEM_DIR, pattern=paste0(TILE,".*.DEM"), full.names = TRUE, recursive=TRUE))
rst.slp <- raster(dir(DEM_DIR, pattern=paste0(TILE,".*.SLOPE"), full.names = TRUE, recursive=TRUE))
...
for(i in 1:nrow(inv.hdffiles)){
  ...
  flag.snow <- stateflags.snow(mdata.mod09@imgvals$state_500m)   
  if (XIAO_SNOWFILTER) flag.snow <- flag.snow + xiaoflags.snow(mdata.indices@imgvals$NDSI,mdata.mod09@imgvals$nir)
  fname.base <- paste(mdata.indices@product, mdata.indices@zone, mdata.indices@acqdate, sep=".")
  for(j in 1:ncol(mdata.indices@imgvals)){
    fname <- paste0(INDICES_DIR,"/", TILE, "/", fname.base, ".", colnames(mdata.indices@imgvals)[j],".tif")
    ...
    thislayer <- writeRaster(thislayer, 
                               ...
                               options = c("COMPRESS=LZW"), datatype="INT2S", overwrite=TRUE)    
    rm(thislayer)
  }
