...
OUTPUT_DIR = "Projects/Heat Stress Models/Output/NighttimeHeat/NP2.30arcmin"
...
if (OUTDEST=="Auto"){
  OUTPUT_DIR <- paste0("./Output/NTH_v", VERSION, "_", WEATHERSET, "_", format(RUNDATE, "%Y%m%d"))
...
for ( i in 1:length(poi)){
  ...
  # tmin should be <= tdew
	...
	wth@lat <- xy[1,"y"]
	wth@w   <- dat.wth
	sowdoy <- rst_plantdoy[poi[i]]
	harvdoy <- rst_harvdoy[poi[i]]
	for (y in years){
	  harvdate <- dateFromDoy(harvdoy, ifelse(sowdoy>harvdoy,y+1, y))
	  flwrdate <- harvdate-30
	  #dstart <- flwrdate-65; dend <- harvdate # PI to maturity
		stress.dat <- rbind(stress.dat, cbind(poi[i], y,ricestress.nighttimeHeat(wth, flwrdate-65, harvdate, year=y, consecutive=FALSE)))		
	}
	mat.tmax <- rbind(mat.tmax, wth@w$tmax)
	mat.tmin <- rbind(mat.tmin, wth@w$tmin)
}
con <- odbcClose(con)
...
for (y in years){		
	...
	for (x in 3:ncol(thisyr)){
	  if(x<12) next
		...
		base <- crop(base,cellListExtent(rst_harvdoy, poi,adjust=2))
		...
		base <- writeRaster(base, filename=paste(OUTPUT_DIR, "/", paste(toupper(colnames(thisyr)[x]), RICE_SEASON, AOI_NAME, y, sep="_"),".tif", sep=""), datatype=dt, format="GTiff", options="COMPRESS=LZW",overwrite=TRUE)
	}				
} 
