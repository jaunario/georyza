    ...
    if (length(to.retain) > 0) {
      ...
      if (length(rice.potential) > 0 && rnr.only) {
        ...
        if (length(rice.potential) > 1) {
          ...
          rice <- sum(10^((seq_along(flooding) - 1) * 3) * flooding + (10^(c(2, 5, 8)[seq_along(flooding)]) * (seq_along(flooding))))
        }
      }
  ...
  if (length(flooding) > 0) {
    ...
    if (length(rice.potential) > 0) {
      if (rnr.only) {
        ...
        rice <- sum(10^((seq_along(true.rice) - 1) * 3) * true.rice + (10^(c(2, 5, 8)[seq_along(true.rice)]) * (seq_along(true.rice))))
      }
    } else {
      rice <- 0
    }
...
crop.signature <- function(vi, dates = null, interval = 1, senescence.min = 5, growth.min = 6, seasonends.pctdiff = 0.1, negligible.change = 100) {
  ...
  trend.senescence <- lapply(trend.senescence, function(x) {
    c(x, x[length(x)] + 1)
  }) # Extends the index to the last value of the trend
  ...
  trend.growth <- lapply(trend.growth, function(x) {
    c(x, x[length(x)] + 1)
  }) # Extends the index to the last value of the trend
  ...
  # lvl1.filter <- mapply("<=", lvl1.match, as.list(seq_along(trend.senescence)), SIMPLIFY = FALSE)
  ...
  #   len.match <- sapply(lvl1.match, length)
  # }
  # lvl2.filter <- mapply("<", lvl1.match, as.list(seq_along(trend.senescence)))
  ...
  # if(sum(len.match)>0){
  #   grps.tocombine <-  mapply(":" , lvl1.match[len.match > 0], as.list((seq_along(trend.senescence))[len.match > 0]), SIMPLIFY = FALSE)
...
rice.itreg <- function(pix.evi, evi.date, dates.covered = NULL, width = 15, increments = 5, eos.evires = 0.4, alpha = 0.05) {
  ...
  if (length(potential.rice) > 0) {
    ...
    for (i in seq_along(potential.rice)) {
