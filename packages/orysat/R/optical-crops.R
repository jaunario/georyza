...
crop.signature <- function(vi, dates = null, interval = 1, senescence.min = 5, growth.min = 6, seasonends.pctdiff = 0.1, negligible.change = 100, data.save = null, as.z = FALSE) {
  ...
  tile <- vi[1]
  tile.h <- trunc(tile)
  tile.v <- (tile - tile.h) * 100
  cell <- vi[2]

  vi <- vi[-(1:2)]
  crops <- NA
  if (sum(is.na(vi)) != length(vi)) {
    ...
    if (!((length(st.growth) == 1 && is.na(st.growth)) || (length(st.senescence) == 1 && is.na(st.senescence)))) {
      ...
      if (length(trend.senescence) > 0 && length(trend.growth) > 0) {
        ...
        if (length(crop.potential) > 0) {
          ...
          rm(trend.crop, sos.evi, evi.amp, evi.ampp5, evi.mid, filter.initevi, filtered.slope)
          ...
          if (length(vi.actualcrop) > 0) {
            ...
            mapply(saveRDS, vi.actualcrop, file = as.list(paste0(data.save, "/", format(sos.date, "m%m/"), sprintf("h%02gv%02g.c%02i.st-%02g.en-%02g.rds", tile.h, tile.v, cell, sos, eos))))

            # for(i in seq_along(vi.actualcrop)){
          ...
          rm(vi.actualcrop, actualcrop.idx, eos, sos)
        }
      }
    }
  }
