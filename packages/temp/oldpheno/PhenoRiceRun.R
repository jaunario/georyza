...
PhenoRiceRun <- function(sub, startYear, tile, mpath=Sys.getenv("MODIS_LOCAL_DIR"), evismoothopt=FALSE, verbose=TRUE, debug=FALSE){
  ...
  if(continua==0){
    maxseason <- 0
	  ...
	  evistackdir <- paste(stackdir, paste("MOD09A1", "../../orysat/R", startYear, ".", tile, ".evi.stack", subs, ".tif", sep=""), sep="/")
	  ndfistackdir <- paste(stackdir, paste("MOD09A1", "../../orysat/R", startYear, ".", tile, ".ndfi.stack", subs, ".tif", sep=""), sep="/")
	  noisestackdir <- paste(stackdir, paste("MOD09A1", "../../orysat/R", startYear, ".", tile, ".noise.stack", subs, ".tif", sep=""), sep="/")
	  lststackdir <- paste(stackdir, paste("MOD11A2", "../../orysat/R", startYear, ".", tile, ".lst.stack", subs, ".tif", sep=""), sep="/")
